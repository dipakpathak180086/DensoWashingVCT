...
{
    public partial class frmConveyorMaster : Form
    {


        #region Variables

        private BL_CONVEYOR_MASTER _blObj = null;
        private PL_CONVEYOR_MASTER _plObj = null;
        ...
        public frmConveyorMaster()
        {
            try
            {
                InitializeComponent();
                _blObj = new BL_CONVEYOR_MASTER();
            }
            catch (Exception ex)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateInput())
                {
                    _plObj = new PL_CONVEYOR_MASTER();
                    _plObj.ConveyorNo = txtConveyorNo.Text.Trim();
                    _plObj.ConveyorName = txtConveyorName.Text.Trim();
        ...
        private void Clear()
        {
            ...
            {
                txtConveyorNo.Text = "";
                txtConveyorName.Text = "";
                txtSeqNo.Text = "";
                chkActive.Checked = false;
                btnDelete.Enabled = false;
                txtConveyorNo.Enabled = true;
        ...
        private bool ValidateInput()
        {
            try
            {
                if (string.IsNullOrEmpty(txtConveyorNo.Text.Trim()))
                {
                    GlobalVariable.mStoCustomFunction.setMessageBox(GlobalVariable.mSatoApps, "Conveyor No can't be blank!!", 3);
                    txtConveyorNo.Focus();
                    txtConveyorNo.SelectAll();
                    return false;
                }
                if (string.IsNullOrEmpty(txtConveyorName.Text.Trim()))
                {
                    GlobalVariable.mStoCustomFunction.setMessageBox(GlobalVariable.mSatoApps, "Conveyor Name can't be blank!!", 3);
                    txtConveyorName.Focus();
                    txtConveyorName.SelectAll();
                    return false;
                }
        ...
        private void dgv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                Clear();
                txtConveyorNo.Text = _partNo = dgv.Rows[e.RowIndex].Cells["ConveyorNo"].Value.ToString();
                txtConveyorName.Text = _partNo = dgv.Rows[e.RowIndex].Cells["ConveyorName"].Value.ToString();
