...
namespace VCTWebApp
{
    public partial class VCTDirectLotInfoReport : System.Web.UI.Page
    {
        ...
        private void _PopulateModel()
        {
            try
            {
                PL_DIRECT_LOT_INFO_REPORT plObj = new PL_DIRECT_LOT_INFO_REPORT();
                BL_DIRECT_LOT_INFO_REPORT blObj = new BL_DIRECT_LOT_INFO_REPORT();
        ...
        private void _PopulateModelName()
        {
            try
            {
                ...
                plObj.ModelNo = ddlModel.SelectedValue;
                DataTable DT = blObj.ShowDetails(plObj);
                if (DT.Rows.Count > 0)
                {
        ...
        private void _GetReportData()
        {
            try
            {
                ...
                plobj.ModelNo = ddlModel.SelectedValue;
                plobj.PartNo = ddlChildPart.SelectedValue;
                plobj.LotNo = txtLotNo.Text;
                DataTable DT = blobj.ShowDetails(plobj);
                if (DT.Rows.Count > 0)
                {
        ...
        private bool _ValidateInput()
        {
            ...
            try
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Please Enter Lot No.');", true);
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Response.Clear();
                    DataTable dt = (DataTable)Session["VCTDirectLotReport"];

                    objclsExportToCSV.ExportTOCSV(dt, "VCTDirectLotReport.csv");
                }
                else
                {
